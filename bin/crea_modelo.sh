...
for mues_ent in `cat $ruta_cli/$1`
  ...
  if test $? -ne 0
    then
    echo ERROR al añadir al fichero de entrenamiento del cliente $cli la muestra $mues_ent 
    exit 1
    fi
   done 
...
if test `expr $num_dat % $tam_vec_carac` -ne 0
  then
  echo "ERROR el numero de datos del fichero $cli.ent (${num_dat}) no es multiplo del tamaño del vector $tam_vec_carac" 
  exit 1
  fi
...
#          while test $cont -le $paso_NTIL  ##linea para insertar un numero fijo, en vez de proporcion
    ...
    if test `expr $num_dat % $tam_vec_carac` -ne 0
      then
      echo "ERROR el numero de datos del fichero $HsIE (${num_dat}) no es multiplo del tamaño del vector $tam_vec_carac" 
      exit 1
      fi
...
### Añado nueva proporcion al fichero de informe
  ...
  if test $? -ne 0
    then
    echo "ERROR al añadir proporcio de la iteracion $it_NTIL a la variable total" 
    exit 1
    fi
